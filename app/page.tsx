...
import { FORCED_VULNERABLE_CARD, PHASE_META, TOTAL_HEARTS_HALF, UNIVERSAL_AWAKENING_CARD, UNIVERSAL_BURST_CARD, getCardsForPhase } from "@/lib/game/data";
import { deriveDistanceLabel, resolvePhaseTurn } from "@/lib/game/phase";
import { pickWeightedRandomCard } from "@/lib/game/random";
import type { CardDefinition, DuelOverlayState, GameContext, GameState, OpponentPersonality, PhaseBannerState, PhaseId, PlayerRoleInPhase, TurnRecord } from "@/lib/game/types";

const INITIAL_PLAYER_X = 2;
const INITIAL_ENEMY_X = 6;
const ENEMY_PERSONALITY: OpponentPersonality = "defensive";
const DUEL_ANIMATION_MS = 3000;

function buildCardPool(phase: PhaseId, role: PlayerRoleInPhase, canUseBurst: boolean, canUseAwakening: boolean) {
...
function buildTurnRecord(prevState: GameState, result: ReturnType<typeof resolvePhaseTurn>, playerCard: CardDefinition, enemyCard: CardDefinition): TurnRecord {
  return {
    turn: prevState.turn,
...
export default function Page() {
  const [state, setState] = useState<GameState>({
    playerHeartsHalf: TOTAL_HEARTS_HALF, enemyHeartsHalf: TOTAL_HEARTS_HALF,
    playerTension: 0, enemyTension: 0, playerX: INITIAL_PLAYER_X, enemyX: INITIAL_ENEMY_X,
    playerPose: "stand", enemyPose: "stand", phase: "opening", playerRoleInPhase: "neutral",
    playerStateText: "뉴트럴", enemyStateText: "뉴트럴", message: "서로 의도를 부딪혔다.",
    commentary: "4x9 전장에서 거리와 높이 판정을 먼저 확인했다.", turn: 1, round: 1,
    effectText: "", enemyPersonality: ENEMY_PERSONALITY, lastPlayerCardId: null, lastEnemyCardId: null,
    playerBurstUsed: false, enemyBurstUsed: false, playerVulnerable: false, enemyVulnerable: false,
  });
  ...
  const currentCards = useMemo(() => {
    ...
    return buildCardPool(state.phase, state.playerRoleInPhase, !state.playerBurstUsed, canUsePlayerAwakening)
  ...
  const distanceLabel = useMemo(() => deriveDistanceLabel(state.playerX, state.enemyX), [state.playerX, state.enemyX]);
  const enemyRole: PlayerRoleInPhase = useMemo(() => state.playerRoleInPhase === "attacker" ? "defender" : state.playerRoleInPhase === "defender" ? "attacker" : "neutral", [state.playerRoleInPhase]);
  ...
  const applyResolvedResult = (prevState: GameState, result: ReturnType<typeof resolvePhaseTurn>, playerCard: CardDefinition, enemyCard: CardDefinition) => {
    ...
    setState((prev) => ({
      ...
      playerX: result.nextPlayerX, enemyX: result.nextEnemyX,
      playerPose: result.nextPlayerPose, enemyPose: result.nextEnemyPose,
      phase: result.nextPhase, playerRoleInPhase: result.nextPlayerRoleInPhase,
      playerStateText: result.nextPlayerStateText, enemyStateText: result.nextEnemyStateText,
      message: result.message, commentary: result.commentary, effectText: result.effectText ?? "",
      turn: prev.turn + 1, lastPlayerCardId: playerCard.id, lastEnemyCardId: enemyCard.id,
      ...
      playerVulnerable: result.nextPlayerVulnerable, enemyVulnerable: result.nextEnemyVulnerable,
    }));
    setDuelOverlay(null);
    setHoveredCard(null);
  ...
  const pickEnemyCard = (snapshot: GameState) => {
    ...
    const pool = buildCardPool(snapshot.phase, enemyRole, !snapshot.enemyBurstUsed, snapshot.enemyTension >= 50)
  ...
  const handleSelectCard = (playerCard: CardDefinition) => {
    ...
    const snapshot: GameState = { ...state, playerTension: spendTensionForCard(state.playerTension, playerCard), enemyTension: spendTensionForCard(state.enemyTension, enemyCardPreview) };
  ...
  const handleReset = () => {
    if (bannerTimerRef.current) clearTimeout(bannerTimerRef.current);
    if (duelApplyTimerRef.current) clearTimeout(duelApplyTimerRef.current);
    setDuelOverlay(null); setHoveredCard(null); setHistory([]);
    setState({
      playerHeartsHalf: TOTAL_HEARTS_HALF, enemyHeartsHalf: TOTAL_HEARTS_HALF,
      playerTension: 0, enemyTension: 0, playerX: INITIAL_PLAYER_X, enemyX: INITIAL_ENEMY_X,
      playerPose: "stand", enemyPose: "stand", phase: "opening", playerRoleInPhase: "neutral",
      playerStateText: "뉴트럴", enemyStateText: "뉴트럴", message: "서로 의도를 부딪혔다.",
      commentary: "4x9 전장에서 거리와 높이 판정을 먼저 확인했다.", turn: 1, round: 1, effectText: "",
      enemyPersonality: ENEMY_PERSONALITY, lastPlayerCardId: null, lastEnemyCardId: null,
      playerBurstUsed: false, enemyBurstUsed: false, playerVulnerable: false, enemyVulnerable: false,
    });
    showBanner("opening");
  };

  const selectionTitle = useMemo(() => state.playerVulnerable ? "무방비 상태" : state.playerRoleInPhase === "attacker" ? "공격자 선택지" : state.playerRoleInPhase === "defender" ? "수비자 선택지" : "뉴트럴 선택지", [state.playerRoleInPhase, state.playerVulnerable]);
  const selectionDescription = useMemo(() => {
    if (state.playerVulnerable) return "잡기/무적/각성 필살기 실패 후 딜레이가 길게 남았다.";
    if (state.playerRoleInPhase === "attacker") return "지금은 네가 흐름을 쥐고 있다. 압박의 방향을 고르자.";
    if (state.playerRoleInPhase === "defender") return "지금은 수세다. 버틸지, 탈출할지, 뒤집을지 고르자.";
  ...
  return (
    <>
      <style jsx global>{`
        ...
        .phase-banner-anim { animation: phaseSlideCenter 1.55s cubic-bezier(0.22, 0.9, 0.22, 1) forwards; }
        .sprite-idle { animation: spriteIdleFrames 0.9s steps(4) infinite; }
        @keyframes spriteIdleFrames { from { background-position: 0 0; } to { background-position: 100% 0; } }
      `}</style>
      ...
      <main className="h-screen overflow-hidden bg-black p-3 text-white sm:p-4">
        <div className="mx-auto flex h-full w-full max-w-[1720px] flex-col">
          <div className="mb-2 flex items-center justify-between">
            <div className="text-[10px] font-black tracking-[0.28em] text-zinc-500 sm:text-xs">TRAINING BUILD V4 · 4x9 GRID · HEART SYSTEM</div>
            <button onClick={handleReset} className="relative z-30 rounded-md border border-zinc-500 bg-zinc-900 px-3 py-2 text-[11px] font-black tracking-[0.12em] hover:border-white sm:text-sm">다시 시작</button>
          </div>
                  ...
                  <div className="flex flex-col gap-3 p-3">
                    ...
                    {!state.playerVulnerable ? (
                      ...
                      <div className="relative z-30 overflow-hidden border border-zinc-700 bg-zinc-950/80 shadow-lg">
                        ...
                        <div className="p-4">
                          <button type="button" onClick={handleAdvanceVulnerableTurn} disabled={isDuelPlaying} className="rounded-md border border-red-500/40 bg-red-950/40 px-4 py-3 text-sm font-black text-red-100 hover:border-red-300 disabled:opacity-40">
                            무방비 턴 진행
                          </button>
                        </div>
                      </div>
