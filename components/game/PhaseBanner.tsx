...
export function PhaseBanner({ banner }: Props) {
  return (
    <div className="pointer-events-none absolute inset-0 z-[60] flex items-center justify-center overflow-hidden">
      <div key={banner.id} className={`phase-banner-anim min-w-[220px] max-w-[88vw] rounded-2xl border px-8 py-4 text-center text-3xl font-black tracking-[0.18em] backdrop-blur-sm ${toneClasses[banner.tone]}`}>
        {banner.text}
      </div>
    </div>
  );
