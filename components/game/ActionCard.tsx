    ...
    >
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="rounded-sm border border-white/10 bg-black/30 px-3 py-1 text-lg font-black tracking-[0.12em] text-white">{card.order}</div>
      ...
      <div className="mb-3 flex flex-wrap gap-1.5">
        {card.tags.map((tag) => (
          <span key={`${card.id}-${tag}`} className="rounded-sm border border-white/10 bg-black/30 px-2 py-1 text-[11px] font-bold tracking-[0.04em] text-zinc-200">
            {tag}
          </span>
        ))}
      </div>
      <div className="mb-4 min-h-[90px] text-sm leading-6 text-zinc-200">{card.description}</div>
      <div className="grid grid-cols-2 gap-2 text-xs text-zinc-300">
        <div className="rounded-sm border border-white/10 bg-black/20 px-2 py-1">전진: {card.advance > 0 ? `+${card.advance}` : card.advance}</div>
        <div className="rounded-sm border border-white/10 bg-black/20 px-2 py-1">밀어내기: {card.push > 0 ? `+${card.push}` : card.push}</div>
        <div className="rounded-sm border border-white/10 bg-black/20 px-2 py-1">공격 범위: {rangeText(card)}</div>
        <div className="rounded-sm border border-white/10 bg-black/20 px-2 py-1">타격 줄: {rowsText(card)}</div>
        <div className="rounded-sm border border-white/10 bg-black/20 px-2 py-1 col-span-2">선택 후 자세: {poseText(card)}</div>
      </div>
    </button>
  );
}
