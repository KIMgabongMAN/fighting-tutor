...
export function ReviewScreen({ history, didWin, onRestart }: Props) {
  const playerSeries = [100, ...history.map((h) => h.playerHpAfter)];
  const enemySeries = [100, ...history.map((h) => h.enemyHpAfter)];
  ...
  const totalDealt = history.reduce(
    (sum, h) => sum + Math.max(0, h.enemyHpBefore - h.enemyHpAfter),
    0
  );
  const totalTaken = history.reduce(
    (sum, h) => sum + Math.max(0, h.playerHpBefore - h.playerHpAfter),
    0
  );
    ...
    <div className="p-4 sm:p-6">
      <div className="mb-5 border border-zinc-700 bg-zinc-950/70 p-5 shadow-xl sm:p-6">
        ...
        <div className="text-sm text-zinc-300 sm:text-base">
          전투 화면을 종료하고 턴별 선택과 체력 흐름을 요약했다.
        </div>
      </div>

      <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatCard label="총 턴 수" value={`${history.length}`} />
        <StatCard label="가한 피해" value={`${totalDealt}`} />
        <StatCard label="받은 피해" value={`${totalTaken}`} />
        ...
        <div className="overflow-hidden rounded-md border border-zinc-800 bg-black/50 p-3 sm:p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-3 text-xs sm:text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className="text-zinc-300">내 HP</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="inline-block h-3 w-3 rounded-full bg-yellow-300" />
              <span className="text-zinc-300">상대 HP</span>
            </div>
          </div>
        </div>
      </div>
          ...
          {history.map((turn) => (
            ...
            >
              ...
              <div className="mb-2 grid gap-2 sm:grid-cols-3">
                <MiniInfo
                  label="내 HP"
                  value={`${turn.playerHpBefore} → ${turn.playerHpAfter}`}
                />
                <MiniInfo
                  label="상대 HP"
                  value={`${turn.enemyHpBefore} → ${turn.enemyHpAfter}`}
                />
                <MiniInfo
                  label="거리"
