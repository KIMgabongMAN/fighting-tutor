...
function situationalBonus(args: WeightedPickArgs, card: CardDefinition) {
  ...
  if (gap >= 5) { if (card.tags.includes("원거리 공격")) bonus += 3; if (card.tags.includes("대쉬")) bonus += 4; }
  if (gap <= 1) { if (card.tags.includes("잡기")) bonus += 3; if (card.tags.includes("근거리 공격")) bonus += 2; }
  if (state.playerPose === "air" && card.tags.includes("대공")) bonus += 4;
  if (state.phase === "pressure" && role === "defender") { if (card.tags.includes("수비")) bonus += 4; }
  if (state.phase === "pressure" && role === "attacker") { if (card.id.includes("frame")) bonus += 3; }
  if (card.tags.includes("버스트")) { if (state.phase === "combo") bonus += 8; else bonus -= 2; }
  return bonus;
}
...
export function pickWeightedRandomCard(args: WeightedPickArgs) {
  ...
  for (const item of weighted) { roll -= item.weight; if (roll <= 0) return item.card; }
